...
function fillInAddress() {
	...
	for (const component of place.address_components) {
		...
		if (componentForm[addressType]) {
			...
			document.getElementById(addressType).value = val.replace("%20", " ");
		}
	}
...
function initMapWithLocations(lat, lng, locationDetails) {
	...
	map.addListener('dragend', function() {
		// 1 second after the center of the map has changed, requery locations based on map center
		window.setTimeout(function() {
			autoQueryMap(map.center.lat(), map.center.lng());
		}, 1000);
	});
